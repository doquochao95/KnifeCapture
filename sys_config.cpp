...
void io_init()
{
    ...
    pinMode(led,OUTPUT);
    pinMode(zero_button,OUTPUT);
    pinMode(GPIO_NUM_13, OUTPUT);
    digitalWrite(GPIO_NUM_13, HIGH);
    digitalWrite(zero_button, LOW);
    motorserial_init();
    nex_init();
...
void PAGE_LOADING_EVENT_CALLBACK(uint8_t pageId, uint8_t componentId, uint8_t eventType)
{
    ...
    case 1: //page setting1
        ...
        if (componentId == BUT_HOME)
        {
            //digitalWrite(zero_button, HIGH);
            motorserial_sendcommand("zero");
            nex_goto_page("LOCK");
            //delay(200);
            //digitalWrite(zero_button, LOW);
            printf("zero motor");
        }
        if (componentId == BUT_INSERT)
        {
            //digitalWrite(zero_button, HIGH);
            motorserial_sendcommand("inse");
            nex_goto_page("LOCK");
            //delay(200);
            //digitalWrite(zero_button, LOW);
            printf("insert knife");
...
bool sys_create_request_timeout_timer()
{
    ...
    RequestTimeOut_TimerHandle = xTimerCreate("RequestTimeOut_TimerHandle", (15000), pdTRUE, NULL, request_timeout_timer);
